...
import { PaginatedData, BahanBaku } from '@/types';

type StokBahanBakuItem = {
    id: number;
...
type Props = {
    stokBahanBaku: PaginatedData<StokBahanBakuItem>;
    kategoriList: string[];
    bahanTanpaStok: BahanBaku[];
    filters: {
        search?: string;
        status?: string;
        kategori?: string;
...
export default function StokBahanBakuIndex({ stokBahanBaku, kategoriList, bahanTanpaStok, filters }: Props) {
    ...
    const [selectedStok, setSelectedStok] = useState<StokBahanBakuItem | null>(null);

    const handleFilterSubmit = (e?: React.FormEvent) => {
        if (e) e.preventDefault();
        router.get('/stok-bahan-baku', {
            search: searchVal,
            status: statusVal,
    ...
    const validateField = (field: string, val: any) => {
        let err = '';
        if (field === 'bahan_baku_id' && !val) {
            err = 'Bahan baku wajib dipilih.';
    ...
    const handleCreateSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        createForm.post('/stok-bahan-baku', {
            onSuccess: () => {
    ...
    const handleEditClick = (item: StokBahanBakuItem) => {
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        ...
        editForm.put(`/stok-bahan-baku/${selectedStok.id}`, {
            onSuccess: () => {
    ...
    const handleImportSubmit = (e: React.FormEvent) => {
        ...
        importForm.post('/stok-bahan-baku/import', {
            onSuccess: () => {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Apakah Anda yakin ingin menghapus konfigurasi stok bahan baku ini?')) {
            router.delete(`/stok-bahan-baku/${id}`);
        }
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            breadcrumb="Kelola ketersediaan logistik bahan baku produksi, safety limits, dan impor berkas"
            headerActions={
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <a
                        href="/stok-bahan-baku/export/excel"
                    ...
                    <a
                        href="/stok-bahan-baku/export/pdf"
        ...
        >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                router.get('/stok-bahan-baku', { search: searchVal, status: statusVal, kategori: e.target.value }, { preserveState: true });
                            }}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                router.get('/stok-bahan-baku', { search: searchVal, status: e.target.value, kategori: kategoriVal }, { preserveState: true });
                            }}
                            className="bg-slate-50 border-1.5 border-gray-100 rounded-lg px-3 py-2 text-xs font-semibold focus:outline-none focus:border-blue-500"
                        >
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {stokBahanBaku.data.length > 0 ? (
                                stokBahanBaku.data.map((item) => (
                                ...
                                <tr>
                                    <td colSpan={8} className="px-6 py-12 text-center text-gray-400">
                                        ...
                                        <p className="text-xs font-semibold">Belum ada data stok bahan baku terdaftar</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                {/* Pagination */}
                {stokBahanBaku.links && stokBahanBaku.links.length > 3 && (
                    <div className="flex justify-between items-center px-6 py-4 bg-slate-50 border-t border-gray-100">
                        <span className="text-xs text-gray-400 font-semibold">
                            Menampilkan {stokBahanBaku.from || 0} - {stokBahanBaku.to || 0} dari {stokBahanBaku.total || 0} data
                        </span>
                        <div className="flex gap-1">
                            {stokBahanBaku.links.map((link, idx) => {
                    ...
                    <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl animate-scale-up border border-gray-100 my-8">
                        <div className="flex justify-between items-center p-5 border-b border-gray-50 bg-slate-50">
                            <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                <i className="ri-add-circle-line text-blue-600 text-lg"></i> Set Stok Bahan Baku Baru
                            </h3>
                            <button
                                onClick={() => {
                                    setIsCreateModalOpen(false);
                        ...
                        <form onSubmit={handleCreateSubmit} className="p-5 space-y-4">
                            <div>
                                ...
                                <select
                                    value={createForm.data.bahan_baku_id}
                                    onChange={(e) => createForm.setData('bahan_baku_id', e.target.value)}
                                    onBlur={(e) => validateField('bahan_baku_id', e.target.value)}
                                    className={`w-full bg-slate-50 border-1.5 border-gray-100 rounded-lg p-2.5 text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all ${
                                        createForm.errors.bahan_baku_id || blurErrors.bahan_baku_id ? 'border-rose-500' : ''
                                    }`}
                                >
                                    <option value="">-- Pilih Bahan Baku --</option>
                                    {bahanTanpaStok.map((b) => (
                                        <option key={b.id} value={b.id}>{b.nama_bahan} ({b.kode_bahan})</option>
                                    ))}
                                </select>
                                {(createForm.errors.bahan_baku_id || blurErrors.bahan_baku_id) && (
                                    <p className="mt-1 text-xs text-rose-500">{createForm.errors.bahan_baku_id || blurErrors.bahan_baku_id}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="contoh: meter, kg, lembar, pcs..."
                                    ...
                                    className={`w-full bg-slate-50 border-1.5 border-gray-100 rounded-lg p-2.5 text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all ${
                                        createForm.errors.satuan || blurErrors.satuan ? 'border-rose-500' : ''
                                    }`}
                                />
                                {(createForm.errors.satuan || blurErrors.satuan) && (
                                    <p className="mt-1 text-xs text-rose-500">{createForm.errors.satuan || blurErrors.satuan}</p>
                                )}
                            </div>
            ...
            {isEditModalOpen && selectedStok && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in overflow-y-auto">
                    <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl animate-scale-up border border-gray-100 my-8">
                        <div className="flex justify-between items-center p-5 border-b border-gray-50 bg-slate-50">
                            <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                <i className="ri-edit-circle-line text-amber-500 text-lg"></i> Edit Parameter Stok
                            </h3>
                            <button
                                onClick={() => {
                                    setIsEditModalOpen(false);
                        ...
                        <form onSubmit={handleEditSubmit} className="p-5 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Nama Bahan Baku</label>
                                <p className="font-bold text-slate-900 text-sm">{selectedStok.bahan_baku?.nama_bahan}</p>
                                <p className="font-mono text-xs text-gray-400 mt-0.5">{selectedStok.bahan_baku?.kode_bahan}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onBlur={(e) => validateField('satuan', e.target.value)}
                                    className={`w-full bg-slate-50 border-1.5 border-gray-100 rounded-lg p-2.5 text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all ${
                                        editForm.errors.satuan || blurErrors.satuan ? 'border-rose-500' : ''
                                    }`}
                                />
                                {(editForm.errors.satuan || blurErrors.satuan) && (
                                    <p className="mt-1 text-xs text-rose-500">{editForm.errors.satuan || blurErrors.satuan}</p>
                                )}
                            </div>
                            ...
                            <div className="flex justify-end gap-2 pt-3 border-t border-gray-100">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg transition-all animate-fade-in"
                                >
            ...
            {isImportModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in overflow-y-auto">
                    <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl animate-scale-up border border-gray-100 my-8">
                        <div className="flex justify-between items-center p-5 border-b border-gray-50 bg-slate-50">
                            <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                <i className="ri-file-upload-line text-blue-600 text-lg"></i> Impor Stok Bahan Baku via Excel
                            </h3>
                            <button
                                onClick={() => {
                                    setIsImportModalOpen(false);
